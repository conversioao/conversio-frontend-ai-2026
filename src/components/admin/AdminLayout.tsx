...
import { LayoutDashboard, Users, CreditCard, Settings, LogOut, Shield, ChevronRight, BarChart2, ShieldAlert, Megaphone, History, Box, Layers, ShieldCheck, TrendingUp, Bot, MessageSquare, ExternalLink } from 'lucide-react';
import AdminMonitorDashboard from './AdminMonitorDashboard';
...
import AdminAgentTeam from './AdminAgentTeam';
...
export function AdminLayout({ currentPage, onNavigate, onLogout, user }: AdminLayoutProps) {
  ...
  React.useEffect(() => {
    ...
    { id: 'admin-agents', icon: <Bot size={20} />, label: 'Equipa de Agentes' },
  ...
  const renderContent = () => {
    switch (currentPage) {
      ...
      case 'admin-agents': return <AdminAgentTeam />;
  ...
  return (
    <div className="min-h-screen bg-bg-base flex selection:bg-accent/30 selection:text-text-primary text-text-primary overflow-hidden">
      ...
      <AnimatePresence>
        {isMonitorOpen && (
          <AdminMonitorDashboard onClose={() => setIsMonitorOpen(false)} />
        )}
      </AnimatePresence>
