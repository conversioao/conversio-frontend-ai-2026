...
export function AdminPlans() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const apiUrl = `http://localhost:3003/api/admin/${endpoint}?adminId=${adminId}`;
  ...
  const handleUpdate = async (item: any, type: 'packages' | 'models') => {
    setSavingId(item.id);
    try {
      const response = await apiFetch(`http://localhost:3003/api/admin/${type}/${item.id}`, {
        method: 'PUT',
  ...
  const handleAdd = async () => {
    try {
      const response = await apiFetch(`http://localhost:3003/api/admin/${activeTab}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (id: string | number, type: 'packages' | 'models') => {
    setModal({
      ...
      onConfirm: async () => {
        try {
          const response = await apiFetch(`http://localhost:3003/api/admin/${type}/${id}?adminId=${adminId}`, {
            method: 'DELETE'
          });
          const result = await response.json();
          if (result.success) {
  ...
  const syncDatabase = async () => {
    setModal({
      ...
      onConfirm: async () => {
        setLoading(true);
        try {
          const resp = await apiFetch(`http://localhost:3003/api/admin/setup`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="relative min-h-[400px]">
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'models' && (
            <motion.div key="models" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                     ...
                     <div className="flex items-center justify-between pt-4 border-t border-border-subtle">
                        <div className="flex items-center gap-2">
                           <div className="flex flex-col">
                              <span className="text-[8px] font-black text-text-tertiary uppercase mb-1">Custo</span>
                 ...
                 {activeTab === 'models' && (
                    <>
                       ...
                       <div className="grid grid-cols-2 gap-4">
                          ...
                          <div><label className="text-[10px] font-black uppercase text-text-tertiary mb-2 block">Custo (Créditos)</label><input type="number" className="bg-bg-base p-4 rounded-2xl w-full border border-border-subtle text-white" value={newItem.credit_cost || 1} onChange={e => setNewItem({...newItem, credit_cost: e.target.value})} /></div>
                       </div>
                    </>
                 )}
