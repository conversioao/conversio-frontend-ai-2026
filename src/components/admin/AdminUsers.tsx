...
export function AdminUsers() {
  ...
  const fetchUsers = async () => {
    try {
      const res = await apiFetch(`http://localhost:3003/api/admin/users?adminId=${adminId}&search=${search}`);
  ...
  const fetchFunnel = async () => {
    try {
      const res = await apiFetch(`http://localhost:3003/api/admin/funnel?adminId=${adminId}`);
      const data = await res.json();
      if (data.success) {
  ...
  const fetchCampaigns = async () => {
    try {
      const res = await apiFetch(`http://localhost:3003/api/admin/campaigns?adminId=${adminId}`);
      const data = await res.json();
      if (data.success) {
  ...
  const fetchUserActivity = async (userId: string) => {
    ...
    try {
      const res = await apiFetch(`http://localhost:3003/api/admin/users/${userId}/activity?adminId=${adminId}`);
      const data = await res.json();
      if (data.success) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await apiFetch(`http://localhost:3003/api/admin/users/${editingUser.id}`, {
        method: 'PUT',
  ...
  const handleDeleteUser = async (userId: string) => {
    setModal({
      ...
      onConfirm: async () => {
        try {
          const res = await apiFetch(`http://localhost:3003/api/admin/users/${userId}?adminId=${adminId}`, {
            method: 'DELETE'
          });
          if (res.ok) {
            fetchUsers();
  ...
  const handleToggleStatus = async (user: any) => {
    ...
    try {
      await apiFetch(`http://localhost:3003/api/admin/users/${user.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleCreateCampaign = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await apiFetch('http://localhost:3003/api/admin/campaigns', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSendCampaign = async (campId: number) => {
    ...
    try {
      const res = await apiFetch('http://localhost:3003/api/admin/campaigns/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
