...
export function AdminModels() {
  ...
  const fetchModels = async () => {
    try {
      ...
      const res = await apiFetch(`http://localhost:3003/api/admin/models?adminId=${adminId}`);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      setCreating(true);
      const adminId = JSON.parse(localStorage.getItem('conversio_user') || '{}').id;
      const res = await apiFetch(`http://localhost:3003/api/admin/models`, {
        method: 'POST',
  ...
  const handleToggle = async (model: any) => {
    try {
      const adminId = JSON.parse(localStorage.getItem('conversio_user') || '{}').id;
      await apiFetch(`http://localhost:3003/api/admin/models/${model.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          adminId, 
  ...
  const handleDelete = async (id: number) => {
    try {
      const adminId = JSON.parse(localStorage.getItem('conversio_user') || '{}').id;
      await apiFetch(`http://localhost:3003/api/admin/models/${id}?adminId=${adminId}`, {
        method: 'DELETE'
      });
