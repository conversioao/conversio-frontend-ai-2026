      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-text-primary tracking-tight flex items-center gap-3">
            <Layers size={32} className="text-[#FFB800]" /> IA e Motores
          </h1>
          <p className="text-text-secondary text-sm font-medium mt-1">
            Gere os cores e modelos ativos. Defina o custo em créditos para cada um.
          </p>
        </div>
        <button
          onClick={handleCleanupStyles}
          disabled={cleaningUp}
          className="flex items-center gap-2 px-5 py-3 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-black uppercase tracking-widest hover:bg-red-500/20 transition-all disabled:opacity-50"
        >
          {cleaningUp ? <RefreshCw size={14} className="animate-spin" /> : <Trash2 size={14} />}
          Limpar Estilos da DB
        </button>
      </div>
