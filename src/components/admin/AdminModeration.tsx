...
export function AdminModeration() {
  ...
  const fetchGenerations = async () => {
    try {
      ...
      const res = await apiFetch(`http://localhost:3003/api/admin/moderation?adminId=${adminId}&page=${page}`);
  ...
  const handleDelete = async (id: string) => {
    setModal({
      ...
      onConfirm: async () => {
        try {
          const adminId = JSON.parse(localStorage.getItem('conversio_user') || '{}').id;
          const res = await apiFetch(`http://localhost:3003/api/generations/${id}?userId=admin&adminId=${adminId}`, { method: 'DELETE' });
          const data = await res.json();
          if (data.success) {
