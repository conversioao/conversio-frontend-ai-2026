  ...
  return (
    ...
    >
      {isVideo ? (
        ...
        <img src={src} alt={title} className="w-full h-full object-cover opacity-80 group-hover:scale-110 transition-transform duration-700" />
      )}
...
export function LandingPage({ onEnter, onNavigate }: LandingPageProps) {
  ...
  const fetchCreditPackages = async () => {
    try {
      const res = await fetch(`http://localhost:3003/api/credit-packages`);
  ...
  return (
    <div ref={containerRef} className="min-h-screen w-full font-sans overflow-x-hidden relative selection:bg-[#FFB800]/30 selection:text-text-primary bg-[#050505] text-text-primary">
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-32 pb-20 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-0 hidden lg:flex">
           <motion.div 
              ...
              className="absolute left-[8%] top-[20%] w-60 aspect-[9/16] rounded-3xl border border-white/10 p-2 bg-white/5 backdrop-blur-md shadow-2xl"
           >
              <img src="/images/hero_product_ugc.png" className="w-full h-full object-cover rounded-2xl opacity-100 transition-all duration-1000" />
           ...
           <motion.div 
              ...
              className="absolute right-[8%] top-[15%] w-72 aspect-square rounded-[3rem] border border-[#FFB800]/20 p-2 bg-[#FFB800]/5 backdrop-blur-md shadow-[0_0_50px_rgba(255,184,0,0.1)]"
           >
              <img src="/images/conv.png" className="w-full h-full object-cover rounded-[2.5rem] opacity-90 transition-all duration-1000" />
           </motion.div>
        </div>
      ...
      {/* ??? 2. PROVA VISUAL IMEDIATA (Exemplos Reais que vendem) */}
        ...
        <div className="container mx-auto px-6 relative z-10">
          <Reveal className="text-center mb-16">
            ...
            <div className="flex bg-surface border border-border-subtle p-1.5 rounded-full mx-auto w-max max-w-full">
              <button onClick={() => setActiveTab('ugc')} className={`px-8 py-3 rounded-full text-sm font-bold transition-all ${activeTab === 'ugc' ? 'bg-text-primary text-bg-base shadow-md' : 'text-text-secondary hover:text-text-primary'}`}>
                UGC Vídeos Virais
              </button>
              <button onClick={() => setActiveTab('3d')} className={`px-8 py-3 rounded-full text-sm font-bold transition-all ${activeTab === '3d' ? 'bg-text-primary text-bg-base shadow-md' : 'text-text-secondary hover:text-text-primary'}`}>
                Imagens de Produto 3D
              </button>
            </div>
          </Reveal>
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'ugc' ? (
              <motion.div key="ugc" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex justify-center gap-6 flex-wrap">
                 <UgcVideoCard src="/videos/ugc.mp4" title="Esse produto mudou a minha rotina!" creator="alimo_angola" isVideo />
                 <UgcVideoCard src="/videos/ugc2.mp4" title="O meu skincare angolano em 30s. ?" creator="nrc_angola" isVideo />
                 <UgcVideoCard src="/videos/ugc_viral_1.mp4" title="Qualidade que fala por si. Incrível." creator="ncr_angola" isVideo />
              </motion.div>
            ) : (
              <motion.div key="3d" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-5xl mx-auto bg-surface/80 backdrop-blur-xl border border-border-subtle rounded-[3rem] p-8 shadow-2xl">
                 <div className="grid md:grid-cols-2 gap-8 items-center">
                    <div className="aspect-square bg-bg-base rounded-2xl relative overflow-hidden group border border-border-subtle">
                       <div className="absolute top-4 left-4 z-20 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold text-white border border-white/10 uppercase">Antes (Sua Foto)</div>
                       <img src="https://images.unsplash.com/photo-1523275335684-37898b6baf30?q=80&w=600&auto=format&fit=crop" className="w-full h-full object-cover grayscale opacity-50 transition-transform duration-1000 group-hover:scale-105" alt="Produto Puro" />
                    </div>
                    <div className="aspect-square bg-bg-base rounded-2xl relative overflow-hidden group shadow-[0_0_40px_rgba(255,184,0,0.15)] border-2 border-[#FFB800]/50">
                       <div className="absolute top-4 right-4 z-20 bg-[#FFB800] px-3 py-1 rounded-full text-xs font-black text-black uppercase shadow-lg flex items-center gap-2"><Sparkles size={14}/> Depois (Gerado em Segundos)</div>
                       <img src="https://images.unsplash.com/photo-1505740420928-5e560c06d30e?q=80&w=600&auto=format&fit=crop" className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" alt="Produto 3D IA" />
                    </div>
                 </div>
              </motion.div>
            )}
      ...
      {/* ?? 3. COMO FUNCIONA (Reduz fricção mental) */}
      ...
      {/* ?? 3. OS 3 CORES SENSACIONAIS */}
      <section id="cores" className="py-32 relative overflow-hidden">
        <div className="container mx-auto px-6 max-w-7xl relative z-10">
          <Reveal className="text-center mb-24">
            <h2 className="text-4xl md:text-6xl font-black text-text-primary tracking-tighter mb-6 leading-tight">
              Os Motores que transformam o seu <br className="hidden md:block"/> produto em <span className="text-[#FFB800]">anúncios que vendem</span>
            </h2>
            <p className="text-xl text-text-secondary max-w-3xl mx-auto font-light">
              Cada Core é um motor de IA especializado. Escolhe o teu e gera anúncios prontos em segundos.
            </p>
          </Reveal>
              ...
              <div className="grid lg:grid-cols-2 gap-16 items-center">
                <div className="order-2 lg:order-1">
                   <div className="flex items-center gap-3 mb-6">
                      ...
                      <h3 className="text-3xl md:text-4xl font-black text-text-primary tracking-tighter">CORE 1 — UGC RealisticLife ??</h3>
                   </div>
                ...
                <div className="order-1 lg:order-2">
                   <BentoCard className="!p-4 bg-surface/30 backdrop-blur-xl border-border-subtle/50">
              ...
              <div className="grid lg:grid-cols-2 gap-16 items-center">
                <div className="hidden lg:block">
                   <BentoCard className="!p-4 bg-surface/30 backdrop-blur-xl border-border-subtle/50">
                      <ComparisonPlaceholder 
                        leftLabel="Foto do Produto" 
                        rightLabel="Banner profissional gerado pela IA" 
                        leftIcon={<Upload size={24} />} 
                        rightIcon={<Layers size={24} />} beforeImage="/antes_brand.png" afterImage="/depois_brand.png" 
                      />
                   </BentoCard>
                </div>
                <div>
                   <div className="flex items-center gap-3 mb-6">
                      ...
                      <h3 className="text-3xl md:text-4xl font-black text-text-primary tracking-tighter">CORE 2 — UGC Influencer ?</h3>
                   </div>
                   <p className="text-xl text-text-secondary font-light mb-8 leading-relaxed">
                     Gera banners e anúncios visuais profissionais mantendo sempre a identidade visual e cores da marca do produto.
                   </p>
                   ...
                   <div className="grid sm:grid-cols-2 gap-6 mb-10">
                      <div className="space-y-4">
                         <h4 className="text-[#FFB800] font-black uppercase text-xs tracking-[0.2em]">Funcionalidades</h4>
                         <ul className="space-y-3">
                            {['Cores exatas do seu produto', 'Estilos Lifestyle ou VFX', '6 formatos (Feed, Stories, etc)'].map(f => (
                               <li key={f} className="flex items-center gap-2 text-sm text-text-primary font-medium">
                                  <CheckCircle2 size={16} className="text-[#FFB800] shrink-0" /> {f}
                               </li>
                            ))}
                      ...
                      <div className="space-y-4">
                         ...
                         <div className="flex flex-wrap gap-2">
                            <span className="px-3 py-1 rounded-full bg-surface border border-border-subtle text-[10px] font-bold text-text-secondary">Nano Banana 2</span>
                            <span className="px-3 py-1 rounded-full bg-surface border border-border-subtle text-[10px] font-bold text-text-secondary">Seedream</span>
                         </div>
                      </div>
                   </div>
                   
                   <button onClick={onEnter} className="group flex items-center gap-2 text-[#FFB800] font-black uppercase text-xs tracking-widest hover:gap-4 transition-all">
                      Explorar UGC Influencer <ArrowRight size={16} />
                   </button>
                </div>
      ...
      {/* ?? 4. O QUE O USUÁRIO PODE CRIAR */}
      <section className="py-32 bg-surface/30 border-y border-border-subtle">
        <div className="container mx-auto px-6 max-w-7xl">
          <Reveal className="mb-20 text-center">
            <h2 className="text-4xl md:text-5xl font-black text-text-primary tracking-tighter mb-4">Tudo o que você precisa num <span className="text-[#FFB800]">só lugar</span></h2>
            <p className="text-xl text-text-secondary font-light">Versatilidade extrema para qualquer operação de vendas.</p>
          </Reveal>

          <div className="grid lg:grid-cols-3 gap-6">
            {[
              { title: "UGC Imagem", desc: "Transforme fotos simples em anúncios de alta conversão com IA de última geração.", img: "/images/agents/ugc_image.png" },
              { title: "UGC Video", desc: "Gere vídeos autênticos com avatares ultrarrealistas perfeitos para Reels e TikTok.", img: "/images/agents/ugc_video.png" },
              { title: "Música", desc: "Trilhas sonoras e vozes exclusivas para dar emoção aos teus criativos num clique.", img: "/images/agents/music.png" }
            ].map((type, i) => (
              <Scroll3DSection key={i}>
                <div className="group relative rounded-[2rem] overflow-hidden bg-surface border border-border-subtle cursor-pointer h-full shadow-lg">
                  <div className="relative aspect-[4/3] overflow-hidden bg-bg-base p-2">
                    <div className="w-full h-full rounded-xl overflow-hidden relative">
                       <img src={type.img} className="w-full h-full object-cover opacity-60 group-hover:scale-110 group-hover:opacity-100 transition-all duration-700" alt={type.title} />
                       <div className="absolute inset-0 bg-gradient-to-t from-bg-base to-transparent relative z-10"></div>
                    </div>
                  </div>
                  <div className="p-8 relative bg-surface border-t border-border-subtle">
                     <h3 className="text-2xl font-bold text-text-primary mb-3 group-hover:text-[#FFB800] transition-colors">{type.title}</h3>
                     <p className="text-text-secondary font-light">{type.desc}</p>
                  </div>
                </div>
              </Scroll3DSection>
            ))}
      ...
      {/* ??? 8. DEMONSTRAÇÃO DO PRODUTO (Novo) */}
      <section className="py-32 bg-surface/30 border-y border-border-subtle">
        <div className="container mx-auto px-6 text-center max-w-6xl">
          ...
          <Reveal>
             <div className="w-full aspect-video bg-bg-base border border-border-subtle rounded-3xl overflow-hidden shadow-2xl relative group cursor-pointer">
                <img src="https://images.unsplash.com/photo-1542744094-24638ea0b3b4?q=80&w=1600&auto=format&fit=crop" className="w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-1000" alt="Plataforma Demo" />
                <div className="absolute inset-0 bg-black/30 group-hover:bg-black/10 transition-colors flex items-center justify-center">
                   <div className="w-24 h-24 rounded-full bg-text-primary/10 backdrop-blur-md border border-white/20 flex flex-col items-center justify-center group-hover:scale-110 shadow-[0_0_50px_rgba(255,184,0,0.3)] transition-all">
              ...
              creditPackages.map((pkg, i) => {
                ...
                return (
                  <Scroll3DSection key={pkg.id}>
                    <div className={`
                      backdrop-blur-xl border p-8 rounded-[2.5rem] relative flex flex-col h-full overflow-hidden transition-all duration-500 bg-[#0A0A0A] border-white/5 hover:border-[#FFB800]/50 group 
                      ${isPopular ? 'border-[#FFB800] shadow-[0_0_40px_rgba(255,184,0,0.15)] scale-[1.03] z-10' : 'shadow-2xl hover:-translate-y-2'}
                    `}>
                      {isPopular && (
                      ...
                      <div className="mb-8 mt-4">
                        <h3 className="text-3xl font-black text-white mb-2 flex flex-wrap items-center gap-3 leading-tight">
                          {pkg.name}
