  ...
  const fetchModels = async () => {
    try {
      ...
      if (coresData.success && coresData.models && coresData.models.length > 0) {
        const mergedCores = coresData.models.map((dbCore: any) => {
          const staticCore = AGENT_CORES.find(c => c.style_id === dbCore.style_id) || AGENT_CORES[0];
          return {
            ...staticCore,
            ...dbCore,
            id: staticCore.id || dbCore.id,
            icon: staticCore.icon || AGENT_CORES[0].icon,
            ...
            <div className="overflow-y-auto px-10 pb-10 flex-1 custom-scrollbar">
              ...
              {wizardStep === 2 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  {AGENT_STYLES[selectedCore?.id || '']?.map((styleName) => (
                    <button
                      key={styleName}
                    ...
                    >
                      ...
                      <span className={`text-[10px] font-medium uppercase tracking-widest ${selectedStyle === styleName ? 'text-[#FFB800]/70' : 'text-text-tertiary'}`}>
                        Estilo Gratuito
                      </span>
                    </button>
                  ))}
