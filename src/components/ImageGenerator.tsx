...
export function ImageGenerator({ initialCore, onClearCore }: ImageGeneratorProps = {}) {
  ...
  const fetchModels = async () => {
    try {
      const modelsRes = await apiFetch('http://localhost:3003/api/models?category=model&type=image');
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await apiFetch('http://localhost:3003/api/generate/image', {
        method: 'POST',
        body: formData
      });
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    setConfirmModalConfig({
      ...
      onConfirm: async () => {
        try {
          const res = await apiFetch(`http://localhost:3003/api/generations/${id}`, {
            method: 'DELETE'
          });
          const data = await res.json();
          if (data.success) {
  ...
  const handlePublish = async (item: any) => {
    ...
    try {
      const res = await apiFetch('http://localhost:3003/api/social/post', {
        method: 'POST',
