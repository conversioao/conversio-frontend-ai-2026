...
import { ImagePlus, Sparkles, Smartphone, Copy, Mic, Send, Download, ThumbsUp, MoreHorizontal, X, Plus, Maximize2, ArrowRight, Globe, Loader2, CheckCircle2, AlertCircle, Zap, Trash2, Layers, ChevronRight, Camera, Package, MessagesSquare, MapPin, TrendingUp, MonitorPlay, BookOpen, Target, Users, Search } from 'lucide-react';
...
const AGENT_CORES = [
  { id: 'ugc-realistic', name: 'UGC RealisticLife', agentTechnicalId: 'ugc-realistic', style_id: 'ugc-realistic', icon: Smartphone, description: 'Conteúdo autêntico e orgânico que converte.', credit_cost: 2 },
  { id: 'boutique-fashion', name: 'Boutique', agentTechnicalId: 'boutique-fashion', style_id: 'boutique-fashion', icon: Sparkles, description: 'Moda, estética e beleza com elegância angolana.', credit_cost: 4 },
  { id: 'impact-ads-pro', name: 'ImpactAds Pro', agentTechnicalId: 'impact-ads-pro', style_id: 'impact-ads-pro', icon: Zap, description: 'Anúncios de alto impacto com inteligência visual.', credit_cost: 5 },
];

// Styles are FREE — only Models and Cores have credit costs
const AGENT_STYLES: Record<string, any[]> = {
  'ugc-realistic': [
    { id: 'Momento Real', name: 'Momento Real', icon: Smartphone, desc: 'Autenticidade total, câmara na mão e vibe orgânica.' },
    { id: 'Unboxing Profissional', name: 'Unboxing Profissional', icon: Package, desc: 'Descoberta do produto com foco em detalhes e surpresa.' },
    { id: 'Depoimento Estratégico', name: 'Depoimento Estratégico', icon: MessagesSquare, desc: 'Conexão emocional direta e plano médio.' },
    { id: 'Estilo de Vida Urbano', name: 'Estilo de Vida Urbano', icon: MapPin, desc: 'Produto no quotidiano luxuoso de Luanda.' },
    { id: 'Performance & Resultado', name: 'Performance & Resultado', icon: TrendingUp, desc: 'Foco no benefício prático e transformação.' },
  ],
  'boutique-fashion': [
    { id: 'Streetwear', name: 'Streetwear', icon: Camera, desc: 'Estilo urbano moderno, vibe de rua premium e lifestyle.' },
    { id: 'Elegante / Formal', name: 'Elegante / Formal', icon: Sparkles, desc: 'Sofisticação extrema, estético e minimalista.' },
    { id: 'Casual / Urbano', name: 'Casual / Urbano', icon: MapPin, desc: 'Vibe do dia-a-dia com estilo e naturalidade.' },
    { id: 'Afro Fashion', name: 'Afro Fashion', icon: Sparkles, desc: 'Identidade africana moderna com cores e padrões vibrantes.' },
    { id: 'Sport / Active', name: 'Sport / Active', icon: Zap, desc: 'Dinâmico, energético e focado em performance.' },
    { id: 'Festa / Gala', name: 'Festa / Gala', icon: Sparkles, desc: 'Glamour, sofisticado e iluminação cinematográfica.' },
  ],
  'impact-ads-pro': [
    { id: 'Outdoor Cinemático', name: 'Outdoor Cinemático', icon: MonitorPlay, desc: 'Foco massivo no produto, título gigante, fundo dramático e de alto contraste.' },
    { id: 'Editorial de Marca', name: 'Editorial de Marca', icon: BookOpen, desc: 'Elegância e simetria de catálogo. Fundo com a cor da marca dominante e texto refinado.' },
    { id: 'Conversão Direta', name: 'Conversão Direta', icon: Target, desc: 'Foco no argumento de venda e call-to-action. Elementos como selos e highlights visuais.' },
    { id: 'Hero Flow', name: 'Hero Flow', icon: Zap, desc: 'Composição energética onde o produto entra em cena de forma dinâmica com o texto.' },
    { id: 'Humano e Emoção', name: 'Humano e Emoção', icon: Users, desc: 'Figura humana atraente (estética angolana) junto do produto, transmitindo ligação genuína.' },
  ],
};
...
export function ImageGenerator({ initialCore, onClearCore, onProgressUpdate }: ImageGeneratorProps = {}) {
  ...
  const [selectedCore, setSelectedCore] = useState<any>(AGENT_CORES[0]); // Default fallback
  const [selectedStyle, setSelectedStyle] = useState<string | null>('Momento Real'); // Default style
  ...
  // Styles are FREE — cost = Model + Core only
  const totalCost = quantity * ((selectedModel?.credit_cost || 1) + (selectedCore?.credit_cost || 0));
  ...
  useEffect(() => {
    ...
    // Fetch latest credits on mount to ensure we aren't using stale data
  ...
  const fetchModels = async () => {
    try {
      ...
      if (modelsData.success) {
        ...
        // Plan filtering removed - all models available to all users with credits
      ...
      if (coresData.success && coresData.models && coresData.models.length > 0) {
        ...
        // Plan filtering removed - all agents available to all users with credits

        const mergedCores = dbCores.map((dbCore: any) => {
          // Robust mapping for legacy IDs and variants
          ...
          return {
            ...staticCore,
            ...dbCore,
            name: staticCore.name, // Force UI name from AGENT_CORES
            // Preserve unique DB ID for selection, use technical ID for engine routing
        ...
        setAvailableCores(mergedCores);
        // Important: Use unique DB id for selection
        setSelectedCore(mergedCores.find((c: any) => c.is_active) || mergedCores[0]);
      }
    } catch (e) {}
  };
  ...
  const handleGenerate = async () => {
    // Image is mandatory for Agent/Standard mode
    if (!selectedCore || (generationMode === 'standard' && !uploadedImage)) return;
    ...
    if (userCredits < totalCost) {
      setConfirmModalConfig({
        ...
        message: `Precisas de ${totalCost} créditos para esta geração, mas tens apenas ${userCredits}. Carrega mais créditos na secção de Faturação.`,
        type: 'warning',
    ...
    // Auto-select random style if not manually set
    let activeStyle = selectedStyle;
    if (!activeStyle && selectedCore) {
      const styles = AGENT_STYLES[selectedCore.agentTechnicalId || selectedCore.id] || [];
      if (styles.length > 0) {
        const firstStyle = styles[0];
        activeStyle = typeof firstStyle === 'string' ? firstStyle : firstStyle.name;
      }
    }
    ...
    if (!isTextOnly) {
      ...
      formData.append('includeText', includeText.toString());
        ...
        if (selectedCore?.agentTechnicalId === 'impact-ads-pro' || selectedCore?.agentTechnicalId === 'boutique-fashion') {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.batchId) connectToProgress(data.batchId);
      } else {
        setConfirmModalConfig({
          title: 'Erro ao Gerar',
      ...
      try {
        ...
        if (data.type === 'progress') {
          ...
          if (data.status === 'completed' || data.status === 'failed') {
            // Fetch current results to append the completed image
            try {
              ...
              if (resData.success) {
                ...
                // Check if all items in this batch are final
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-9rem)] w-full max-w-6xl mx-auto relative px-4 sm:px-0">
      ...
      {/* ─── Real-time Generation Progress Overlay (REMOVED: Now inline) ─── */}
      ...
      {(status === 'done' || (status === 'generating' && sseProgress)) && (
        <div className="w-full max-w-4xl mx-auto mt-4 mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className={`grid gap-4 ${status === 'generating' || generatedItems.length === 1 ? 'grid-cols-1 max-w-sm mx-auto' : generatedItems.length === 2 ? 'grid-cols-2' : 'grid-cols-2 sm:grid-cols-3'}`}>
            
            {/* Loading Placeholder Card */}
            {status === 'generating' && sseProgress && (
              <div className="group relative rounded-2xl overflow-hidden border border-accent/30 bg-surface/50 p-6 shadow-2xl flex flex-col items-center justify-center gap-4 animate-pulse-glow"
                ...
                <div className="text-center space-y-1">
                  <p className="text-text-primary font-bold text-sm tracking-tight">{sseProgress.label}</p>
                  <p className="text-[10px] text-accent font-black uppercase tracking-widest">{sseProgress.percent}% CONCLUÍDO</p>
                </div>
            ...
            {generatedItems.map((item: any) => (
              <div key={item.id} className="group relative rounded-2xl overflow-hidden border border-border-subtle bg-surface hover:border-accent/30 transition-all duration-300 shadow-lg hover:shadow-xl hover:shadow-accent/5">
                {item.status === 'completed' && item.result_url ? (
                  <>
                    <img
                      src={item.result_url}
                      alt={item.prompt || 'Geração'}
                      className="w-full aspect-square object-cover cursor-pointer hover:scale-[1.02] transition-transform duration-500"
                      onClick={() => setExpandedImage(item)}
                      onError={(e) => { (e.target as HTMLImageElement).src = 'https://via.placeholder.com/400?text=Imagem+Indisponível'; }}
                    />
                    {/* Action overlay on hover */}
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-end justify-center gap-2 p-3">
                      <button
                        onClick={() => setExpandedImage(item)}
                        className="flex-1 py-2 rounded-xl bg-white/10 backdrop-blur-md text-white text-xs font-semibold border border-white/20 hover:bg-white/20 transition-all flex items-center justify-center gap-1.5"
                      >
                        <Maximize2 size={13} /> Ver
                      </button>
                      <a
                        ...
                        className="flex-1 py-2 rounded-xl bg-accent/90 text-black text-xs font-semibold hover:bg-accent transition-all flex items-center justify-center gap-1.5"
                      >
                    ...
                    {/* Copy badge */}
                    {item.metadata?.copy && (
                      <div className="absolute top-2 left-2 right-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
                        <div className="bg-black/80 backdrop-blur-md rounded-xl p-2 text-white text-[10px] leading-relaxed line-clamp-2">
                          {item.metadata.copy}
                        </div>
                      </div>
                    )}
                  ...
                  <div className="aspect-square flex flex-col items-center justify-center bg-surface-hover text-text-secondary gap-2">
                    <AlertCircle size={24} className="text-red-400" />
                    <p className="text-xs font-medium">Falha na geração</p>
                    {item.metadata?.error && (
                      <p className="text-[10px] text-text-tertiary px-4 text-center line-clamp-2">{item.metadata.error}</p>
                    )}
                  </div>
                )}
              </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                 ...
                 <div className="flex flex-col items-end gap-1">
                    <button 
                      ...
                      disabled={!selectedCore || (generationMode === 'standard' && !uploadedImage)}
                ...
                <div className="flex items-center gap-4 mb-1">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-white tracking-tight">
                      ...
                      {wizardStep === 2 && <>Configurar <span className="text-[#FFB800]">Design</span></>}
                    </h2>
                  </div>
                </div>
              </div>
            ...
            <div className="overflow-y-auto px-10 pb-10 flex-1 custom-scrollbar">
              ...
              {wizardStep === 1 && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  {availableCores.map((core: any) => {
                    // Use unique database id for selection
              ...
              {wizardStep === 2 && selectedCore && AGENT_STYLES[selectedCore.agentTechnicalId] && (
                <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 mb-8">
                  <h4 className="text-xs font-semibold text-text-tertiary uppercase tracking-widest px-1">
                    Selecione o Estilo {selectedCore.name.includes('UGC') ? 'UGC' : 'Visual'}
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {AGENT_STYLES[selectedCore.agentTechnicalId].map((s: any) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex flex-col items-center text-center p-5 rounded-2xl border transition-all ${
                            isSelected 
                              ? 'bg-[#FFB800]/10 border-[#FFB800]/40 text-[#FFB800] shadow-[0_0_20px_rgba(255,184,0,0.15)]' 
                              : 'bg-white/[0.02] border-white/5 text-text-tertiary hover:border-white/10 hover:bg-white/[0.04]'
                          }`}
                        >
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 transition-colors ${isSelected ? 'bg-[#FFB800] text-black' : 'bg-white/5 group-hover:text-white'}`}>
                            <Icon size={20} />
                          </div>
                          <span className="text-sm font-bold block mb-1">{s.name}</span>
                          {s.desc && <span className="text-[10px] font-normal opacity-60 leading-tight">{s.desc}</span>}
                        </button>
                      );
                    })}
                  </div>
              ...
              {wizardStep === 2 && (
                <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  {(selectedCore?.agentTechnicalId === 'impact-ads-pro' || selectedCore?.agentTechnicalId === 'boutique-fashion') && (
                    <div className="flex flex-col gap-4 p-6 rounded-[2rem] bg-accent/5 border border-accent/20 animate-in fade-in slide-in-from-bottom-2 duration-500">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-text-tertiary">Escolha a paleta cromática para o {selectedCore.name}</p>
                        </div>
                      </div>
