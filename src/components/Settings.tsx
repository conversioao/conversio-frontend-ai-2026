...
export function Settings() {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Show local preview immediately (DO NOT auto-analyze)
  ...
  const handleAnalyzeClick = async () => {
    ...
    try {
      ...
      setError('Erro crítico ao conectar ao servidor de análise.');
    } finally {
      setAnalyzing(false);
    }
  };
            ...
            <div className="flex flex-col gap-6">
              ...
              {logoFile && !analyzing && (
                <button 
                  ...
                  className="w-full py-4 bg-accent text-black font-semibold rounded-2xl text-[11px] uppercase tracking-widest hover:scale-[1.02] shadow-[0_10px_30px_rgba(255,184,0,0.2)] transition-all flex items-center justify-center gap-3 animate-in fade-in slide-in-from-bottom-2"
                >
                  <Sparkles size={18} /> Extrair Identidade Visual
                </button>
              )}
              ...
              {analyzing && (
                <div className="bg-accent/5 border border-accent/10 rounded-2xl p-4 flex items-start gap-3">
                  ...
                  <p className="text-accent text-[10px] font-medium leading-relaxed">
                    Estamos a extrair automaticamente a paleta de cores e o nome da marca a partir do seu logótipo através do webhook.
                  </p>
                </div>
              )}
            </div>
