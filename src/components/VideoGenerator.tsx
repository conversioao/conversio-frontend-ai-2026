...
const VIDEO_CORES = [
  { id: 'ugc-influencer-video', name: 'UGC Influencer', agentTechnicalId: 'ugc-influencer-video', icon: Smartphone, description: 'Estilo autêntico para Reels e TikTok que gera confiança imediata.', credit_cost: 2 },
  { id: 'lifestyle-aspiracional-video', name: 'Lifestyle Aspiracional', agentTechnicalId: 'lifestyle-aspiracional-video', icon: MonitorPlay, description: 'Vídeos dinâmicos que conectam o produto ao estilo de vida angolano premium.', credit_cost: 5 },
        ...
        <div className="w-full max-w-4xl mx-auto mt-4 mb-8 animate-in fade-in slide-in-from-top-4 duration-500">
          ...
          <div className="max-w-md mx-auto relative">
            {status === 'generating' && sseProgress ? (
              <div className="aspect-[9/16] rounded-3xl bg-surface/50 border border-[#FFB800]/20 flex flex-col items-center justify-center gap-8 p-10 relative overflow-hidden shadow-2xl">
                  <div className="w-24 h-24 relative">
                    ...
                    <Video className="absolute inset-0 m-auto text-[#FFB800]" size={40} />
                  </div>
                  <div className="text-center space-y-2">
                    <p className="text-lg font-black text-text-primary tracking-tight">{sseProgress.label}</p>
                    <p className="text-xs text-[#FFB800] font-black uppercase tracking-[0.3em]">{sseProgress.percent}%</p>
                  </div>
                  <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden">
                    <div className="h-full bg-[#FFB800] transition-all duration-700 shadow-[0_0_15px_#FFB800]" style={{ width: `${sseProgress.percent}%` }} />
                  </div>
              </div>
            ) : (
                <div className="grid grid-cols-1 gap-6">
                    {generatedItems.map((item, idx) => (
                        <div key={item.id} className="rounded-3xl overflow-hidden shadow-[0_40px_80px_rgba(0,0,0,0.5)] border border-white/10 relative group/result">
                             {item.status === 'completed' ? (
                                <>
                                    ...
                                    <div className="absolute top-6 right-6 flex flex-col gap-3">
                                        <a href={item.result_url} download className="p-4 rounded-2xl bg-[#FFB800] text-black hover:scale-110 transition-transform shadow-2xl">
                                            <Download size={22} />
                                        </a>
                                    </div>
                                </>
                             ) : (
                                <div className="aspect-[9/16] flex flex-col items-center justify-center bg-surface gap-6 p-10 text-center">
                                    <AlertCircle size={50} className="text-red-500" />
                                    <p className="text-xl font-black text-text-primary">A geração falhou</p>
                                    <button onClick={() => setStatus('idle')} className="mt-4 px-10 py-4 rounded-2xl bg-white/5 border border-white/10 text-sm font-black uppercase tracking-widest">Tentar Novamente</button>
                                </div>
                             )}
                        </div>
                    ))}
