...
const API_URL = (import.meta.env.VITE_API_URL || 'https://conversioai-conversio-ai-backend.odbegs.easypanel.host/api');
...
export function VideoGenerator({ initialCore, onClearCore }: VideoGeneratorProps = {}) {
  ...
  const fetchModels = async () => {
    try {
      const [modelsRes, stylesRes, coreRes] = await Promise.all([
        apiFetch(`${API_URL}/models?type=video&category=model`),
        apiFetch(`${API_URL}/models?type=video&category=style`),
        apiFetch(`${API_URL}/models?type=video&category=core`)
      ]);
  ...
  const handleOpenGallery = async () => {
    ...
    try {
      const res = await apiFetch(`${API_URL}/generations?userId=${user.id}`);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await apiFetch(`${API_URL}/generate/video`, { method: 'POST', body: formData });
      const data = await res.json();
