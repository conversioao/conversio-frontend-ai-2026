...
const VIDEO_CORES = [
  { id: 'campanha-institucional', name: 'Campanha Institucional', desc: 'Ideal para vídeos de marca e institucionais.', credit_cost: 5 },
  { id: 'antes-depois-visual', name: 'Antes e Depois Visual', desc: 'Perfeito para mostrar transformações e resultados.', credit_cost: 5 },
  { id: 'problema-solucao-lifestyle', name: 'Problema Solução ResultadoLifestyle', desc: 'Foco em dor, solução e lifestyle do cliente.', credit_cost: 5 },
  { id: 'cinematic-product-hero', name: 'Cinematic Product Hero', desc: 'Vídeos de produto com alta fidelidade cinematográfica.', credit_cost: 5 },
  { id: 'ugc-influencer', name: 'UGC Influencer', desc: 'Estilo autêntico para redes sociais e influenciadores.', credit_cost: 5 }
];
...
export function VideoGenerator({ initialCore, onClearCore }: VideoGeneratorProps = {}) {
  ...
  const fetchModels = async () => {
    try {
      ...
      if (coreData.success && coreData.models) {
        const mergedCores = coreData.models.map((dbCore: any) => {
          const staticCore = VIDEO_CORES.find(c => c.id === dbCore.style_id) || VIDEO_CORES[0];
          return {
            ...staticCore,
            ...dbCore,
            id: staticCore.id || dbCore.id, // keep simple ID for styles map lookup
            desc: staticCore.desc || VIDEO_CORES[0].desc,
          };
        });
        setAvailableCoreModels(mergedCores);
        if(!selectedCore) {
            setSelectedCore(mergedCores[0]);
        }
      }
    } catch (e) {}
  };
  ...
  const handleGenerate = async () => {
    ...
    const totalCost = quantity * ((selectedModel?.credit_cost || 10) + (selectedCore?.credit_cost || 0));
            ...
            <div className="overflow-y-auto px-10 pb-10 flex-1 custom-scrollbar">
              ...
              {wizardStep === 1 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  {availableCoreModels.map((core: any) => {
              ...
              {wizardStep === 2 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  {selectedCore && selectedCore.id && VIDEO_STYLES_MAP[selectedCore.id]?.map((styleName: string) => (
                    <button
                      key={styleName}
                    ...
                    >
                      ...
                      <span className={`text-[10px] font-medium uppercase tracking-widest ${selectedStyle === styleName ? 'text-[#FFB800]/70' : 'text-text-tertiary'}`}>Estilo Gratuito</span>
                    </button>
                  ))}
                  <button
