        ...
        <div className="relative rounded-[2rem] md:rounded-full p-[1px] overflow-hidden shadow-float w-[70vw] md:w-[60px] max-w-[280px]">
          ...
          <aside className="relative bg-surface/95 backdrop-blur-xl rounded-[calc(2rem-1px)] md:rounded-[calc(9999px-1px)] py-6 md:py-3 px-4 md:px-1.5 flex flex-col items-center gap-5 border border-border-subtle h-[75vh] md:h-auto overflow-y-auto custom-scrollbar">
            ...
            <div className="flex flex-col gap-3 w-full md:w-auto items-center">
              {[
                { id: 'home', icon: <Home size={20} strokeWidth={activePage === 'home' ? 2 : 1.5} />, label: 'Início', visible: true },
                { id: 'generate-image', icon: <ImageIcon size={20} strokeWidth={activePage === 'generate-image' ? 2 : 1.5} />, label: 'Gerar Imagem', visible: true },
                { id: 'editor', icon: <PenTool size={20} strokeWidth={activePage === 'editor' ? 2 : 1.5} />, label: 'Editor Pro', visible: planConfig.hasEditor },
                { id: 'generate-video', icon: <Video size={20} strokeWidth={activePage === 'generate-video' ? 2 : 1.5} />, label: 'Gerar Vídeo', visible: planConfig.hasMediaGen },
                { id: 'generate-audio', icon: <Music size={20} strokeWidth={activePage === 'generate-audio' ? 2 : 1.5} />, label: 'Gerar Música', visible: planConfig.hasMediaGen },
                { id: 'audio-gallery', icon: <Headphones size={20} strokeWidth={activePage === 'audio-gallery' ? 2 : 1.5} />, label: 'Biblioteca Áudio', visible: planConfig.hasMediaGen },
                { id: 'projects', icon: <Folder size={20} strokeWidth={activePage === 'projects' ? 2 : 1.5} />, label: 'Projetos', visible: true },
                { id: 'billing', icon: <CreditCard size={20} strokeWidth={activePage === 'billing' ? 2 : 1.5} />, label: 'Créditos', visible: true },
