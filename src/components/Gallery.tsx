...
export function Gallery() {
  ...
  const fetchGenerations = async (page = 1) => {
    ...
    try {
      const response = await apiFetch(`http://localhost:3003/api/generations?userId=${user.id}&page=${page}&limit=18&excludeTypes=audio,voice,musica`);
  ...
  const handleDelete = React.useCallback(async (e: React.MouseEvent, id: string) => {
    ...
    setModal({
      ...
      onConfirm: async () => {
        try {
          const response = await apiFetch(`http://localhost:3003/api/generations/${id}`, {
            method: 'DELETE'
          });
