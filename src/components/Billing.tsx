...
export function Billing() {
  ...
  const fetchData = async () => {
    try {
      const [sRes, tRes, cpRes, pRes] = await Promise.all([
        apiFetch(`http://localhost:3003/api/user/stats?userId=${localUser.id}`),
        apiFetch(`http://localhost:3003/api/user/transactions?userId=${localUser.id}`),
        apiFetch(`http://localhost:3003/api/credit-packages`),
        apiFetch(`http://localhost:3003/api/payment-info`)
      ]);
  ...
  const handleCheckoutSubmit = async () => {
    ...
    try {
      ...
      const res = await apiFetch('http://localhost:3003/api/billing/checkout', {
        method: 'POST',
      ...
      if (data.success && data.transactionId) {
        ...
        const uploadRes = await apiFetch('http://localhost:3003/api/billing/upload-proof', {
          method: 'POST',
          body: formData
        });
    ...
    <div className="flex flex-col w-full max-w-6xl mx-auto animate-in fade-in duration-500 pb-10">
                 ...
                 >
                    ...
                    <div className="mb-10 flex-grow">
                        ...
                        <ul className="space-y-4">
                            ...
                            <li className="flex items-start gap-3 text-sm">
                                <Check size={18} className="text-[#FFB800] mt-0.5 shrink-0" strokeWidth={3} />
                                <span className="text-text-secondary text-white font-bold">Acesso a todos os Motores AI</span>
                            </li>
                        </ul>
                    </div>
      ...
      <section className="bg-surface/80 backdrop-blur-xl border border-border-subtle rounded-[2rem] overflow-hidden shadow-lg">
        <div className="p-6 border-b border-border-subtle flex items-center justify-between">
          <h2 className="text-lg font-semibold text-text-primary">Histórico de Transações</h2>
                     ...
                     <td className="px-6 py-4">
                        {t.invoice_url ? (
                          <a 
                            ...
                            className="flex items-center gap-1.5 text-xs font-bold text-[#FFB800] hover:text-[#FFB800]/80 transition-colors bg-[#FFB800]/10 px-3 py-1.5 rounded-lg border border-[#FFB800]/20"
                          >
                            <ReceiptText size={14} />
                            Fatura
                          </a>
