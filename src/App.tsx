...
export default function App() {
  ...
  const fetchBrand = async () => {
    ...
    if (!user.id) return;
    try {
      const res = await apiFetch(`http://localhost:3003/api/brands/${user.id}`);
  ...
  const fetchStats = async () => {
    ...
    if (!user.id) return;
    try {
      const res = await apiFetch(`http://localhost:3003/api/user/stats?userId=${user.id}`);
      const data = await res.json();
      if (data.success) {
  ...
  const fetchSocialNotifications = async () => {
    ...
    if (!user.id) return;
    try {
      const res = await apiFetch(`http://localhost:3003/api/social/notifications?userId=${user.id}`);
      const data = await res.json();
      if (data.success) {
  ...
  const fetchBroadcast = async () => {
    try {
      const res = await apiFetch(`http://localhost:3003/api/admin/broadcasts`);
      const data = await res.json();
  ...
  const handleNotificationClick = async () => {
    ...
    if (socialNotifications.some((n: any) => !n.is_read)) {
      ...
      const user = userStr ? JSON.parse(userStr) : {};
      try {
        await apiFetch(`http://localhost:3003/api/social/notifications/read`, {
          method: 'POST',
    ...
    setConfirmModalConfig({
      ...
      onConfirm: async () => {
        try {
          ...
          const response = await apiFetch(`http://localhost:3003/api/generations/${id}?userId=${user.id}`, {
            method: 'DELETE'
          });
  ...
  const handlePublish = async (item: any) => {
    ...
    try {
      const response = await apiFetch('http://localhost:3003/api/social/post', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: user.id,
