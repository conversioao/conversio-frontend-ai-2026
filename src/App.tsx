...
const AudioGenerator = lazy(() => import('./components/AudioGenerator').then(m => ({ default: m.AudioGenerator })));
const AudioGallery = lazy(() => import('./components/AudioGallery').then(m => ({ default: m.AudioGallery })));
...
const ProEditor = lazy(() => import('./components/ProEditor').then(m => ({ default: m.ProEditor })));
const VideoCoresPage = lazy(() => import('./components/VideoCoresPage').then(m => ({ default: m.VideoCoresPage })));
const CoresPage = lazy(() => import('./components/CoresPage').then(m => ({ default: m.CoresPage })));
...
export default function App() {
  const [currentPage, setCurrentPage] = useState(() => {
    ...
    // Se não há autenticação, prioriza a landing page como inicial
    if (!token || !user) {
      if (isLaunchedFromPWA()) {
        // No PWA, se houver página salva (como auth), respeita, senão landing
        return savedPage || 'landing';
      }
      // No browser normal, sempre landing ou auth se explicitamente salvo, 
      // mas se estivermos a abrir a app pela primeira vez (sem savedPage), é landing.
      if (savedPage && (savedPage === 'landing' || savedPage === 'auth')) return savedPage;
      return 'landing';
    }

    // Se está autenticado, volta para onde estava ou home
  ...
  const fetchRecent = async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.generations)) {
        // Derive generating status based on processing items
        setIsGeneratingImage(data.generations.some((g: any) => g.status === 'processing' && g.type === 'image'));
        setIsGeneratingVideo(data.generations.some((g: any) => g.status === 'processing' && g.type === 'video'));
        setIsGeneratingAudio(data.generations.some((g: any) => g.status === 'processing' && g.type === 'audio'));
        setRecentGenerations(data.generations);

        // Split recents into media and audio
        const mediaGens = data.generations.filter((g: any) => g.type === 'image' || g.type === 'video');
        const audioGens = data.generations.filter((g: any) => g.type === 'audio' || g.type === 'voice' || g.type === 'musica');
        const newMediaGens = mediaGens.slice(0, 5);
        setRecentAudioGenerations(audioGens.slice(0, 3));
  ...
  React.useEffect(() => {
    ...
    return () => window.removeEventListener('conversio_navigate', handleCustomNav);
  }, []);
  ...
  const handleNavigate = (page: string) => {
    ...
    // Soft haptic feedback on navigation
    triggerHaptic(hapticPatterns.short);
    if (page === 'home') {
      fetchStats();
      fetchRecent();
    }
  ...
  const handleNotificationClick = async () => {
    ...
    if (socialNotifications.some((n: any) => !n.is_read)) {
      ...
      try {
        ...
        // Update local state
        setSocialNotifications(prev => prev.map(n => ({ ...n, is_read: true })));
      } catch (e) {}
    }
  };
  ...
  if (currentPage.startsWith('admin')) {
    ...
    return (
      <Suspense fallback={<LoadingScreen />}>
        <AdminLayout currentPage={currentPage} onNavigate={handleNavigate} onLogout={handleLogout} user={user} />
      </Suspense>
    );
  }
  ...
  if (currentPage === 'landing') {
    return (
      <Suspense fallback={<LoadingScreen />}>
        <LandingPage onEnter={() => handleNavigate('auth')} onNavigate={handleNavigate} />
      </Suspense>
    );
  }
  ...
  if (currentPage === 'video-cores-info') {
    return (
      <Suspense fallback={<LoadingScreen />}>
        <VideoCoresPage onEnter={() => handleNavigate('auth')} onBack={() => handleNavigate('landing')} />
      </Suspense>
    );
  }
  ...
  if (currentPage === 'image-cores-info') {
    return (
      <Suspense fallback={<LoadingScreen />}>
        <CoresPage onEnter={() => handleNavigate('auth')} onBack={() => handleNavigate('landing')} />
      </Suspense>
    );
  }
  ...
  const renderContent = () => {
          ...
          <section className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              >
                ...
                <div className="relative z-10 flex flex-col h-full bg-surface/95 backdrop-blur-xl rounded-[calc(2.5rem-1px)] p-6 text-left border border-white/5">
                  <div className="absolute top-0 right-0 w-64 h-64 bg-accent/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 group-hover:bg-accent/10 transition-colors"></div>
                  <div className="w-12 h-12 rounded-2xl bg-bg-base border border-border-subtle flex items-center justify-center text-text-primary mb-4 group-hover:scale-110 transition-transform shadow-inner relative z-20 shrink-0">
                    <Mic size={24} strokeWidth={1.5} />
                  </div>
                  <h3 className="text-xl font-bold text-text-primary mb-2 relative z-20">Geração de Voz</h3>
                  <p className="text-text-secondary text-sm mb-6 max-w-sm leading-relaxed relative z-20">Converta scripts em dublagens profissionais com clones de voz realistas.</p>
                  <div className="mt-auto flex items-center gap-2 text-sm font-medium text-text-primary relative z-20">
    ...
    if (currentPage === 'generate-image') {
      return <ImageGenerator initialCore={selectedCore} onClearCore={() => setSelectedCore(null)} />;
    }
